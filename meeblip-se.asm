...
DCA:
			...
			mulsu	r17, r18		; multiply sample's high byte
			movw	r30, r0
			mul		r16, r18		; multipliy sample's low byte
			add		r30, r1
			adc		r31, ZERO
T2_AEXIT:
...
;initialize sound parameters:
		    ldi	    r16,0
			sts		KNOB0_STATUS, r16	; No knobs have been moved yet
			sts		KNOB1_STATUS, r16	; No knobs have been moved yet
			sts		KNOB2_STATUS, r16	; No knobs have been moved yet
			sts		KNOB3_STATUS, r16	; No knobs have been moved yet
			sts		KNOB4_STATUS, r16	; No knobs have been moved yet
			sts		KNOB5_STATUS, r16	; No knobs have been moved yet
			sts		KNOB6_STATUS, r16	; No knobs have been moved yet
			sts		KNOB7_STATUS, r16	; No knobs have been moved yet
		    sts	    LFOPHASE, r16		;
			sts	    LFO2PHASE, r16		;
		    sts	    ENVPHASE, r16		;
...
; Load saved knob parameter values from eeprom at power up

			in 		r19, SREG 				; store SREG value
			cli
			ldi		r18, 0
			ldi		r17, 0					; Set eeprom memory offset to zero, then read 8 bytes...
			rcall	EEPROM_read
			sts 	KNOB_DCF_DECAY, r16		; Byte 0
			inc		r17
			rcall	EEPROM_read
			sts		KNOB_DCF_ATTACK, r16	; Byte 1
			inc		r17
			rcall	EEPROM_read
			sts		KNOB_AMP_DECAY, r16		; Byte 2
			inc		r17
			rcall	EEPROM_read
			sts 	KNOB_AMP_ATTACK, r16	; Byte 3
			inc		r17
			rcall	EEPROM_read
			sts		LFOFREQ, r16			; Byte 4
			inc		r17
			rcall	EEPROM_read
			sts		PANEL_LFOLEVEL, r16		; Byte 5
			inc		r17
			rcall	EEPROM_read
			sts		VCFENVMOD, r16			; Byte 6
			inc		r17
			rcall	EEPROM_read
			sts		PORTAMENTO, r16			; Byte 7
			out 	SREG, r19 				; restore SREG value (I-bit)
...
;store initial pot positions as OLD_ADC values to avoid snapping to new value unless knob has been moved.
		    ...
		    rcall	ADC_START	        ; start conversion of ADC0 for main loop 
										...
										; Load the current single parameter knob positions as the parameter values	
			lds	    r16, ADC_0			; Save knob positions in OLD_ADC_X for future comparison
			sts	    OLD_ADC_0,r16
			sts		RESONANCE,r16		; Load knob 0 value into RESONANCE, etc...
			lds	    r16, ADC_1			 
			sts	    OLD_ADC_1,r16
			sts		CUTOFF,r16			; Cutoff
			lds	    r16, ADC_2			 
			sts	    OLD_ADC_2,r16
			lds	    r16, ADC_3			 
			sts	    OLD_ADC_3,r16	
			...
			lds	    r16, ADC_6			 
			sts	    OLD_ADC_6,r16
			sts		PULSE_KNOB,r16		; PWM
			lds	    r16, ADC_7			 
			sts	    OLD_ADC_7,r16
			sts		OSC_DETUNE,r16		; Osc detune
...
; capture switch values on power up
			...
			sts		OLD_SWITCH2, r20
			sts	    PATCH_SWITCH1, r19	; Read starup patch switch values from the front panel
		    sts	    PATCH_SWITCH2, r20
...
MLP_PATCH1SAVE:	
			sts		PATCH_SWITCH1, r18		; Store changes	
			sts		OLD_SWITCH1, r18		; Keep a copy so we know if things change next time
MLP_SWITCH2:				
...
MLP_PATCH2SAVE:			
			sts		PATCH_SWITCH2, r18		; Store changes	
			sts		OLD_SWITCH2, r18		; Keep a copy so we know if things change next time
			
MLP_SW_EXIT:


; Set MIDI channel based on switches:
...
; Check if knob shift switch has changed:
			...
			lds		r19, PATCH_SWITCH1	; Grab SWITCH1 for checking shift switch

; At power up, set previous knob shift value to current switch setting and jump to read knobs
			lds		r17, POWER_UP		; Is this the first time through this code since synth was turned on?
			sbrs	r17, 0				; No: skip to read the 'knob shift' switch and see if it's changed.
			rjmp	MLP_SHIFT			
			sbrs	r19, SW_KNOB_SHIFT	; Yes: Store current knob shift switch value as previous value
			ldi		r16, 0				; Test if 'knob shift' bit is set
			sbrc	r19, SW_KNOB_SHIFT
			ldi		r16, 1
			sts		KNOB_SHIFT, r16		 
			clr		r17					
...
MLP_SHIFT:
			sbrs	r19, SW_KNOB_SHIFT	; Test if 'knob shift' bit is set
			ldi		r16, 0
			sbrc	r19, SW_KNOB_SHIFT
			ldi		r16, 1
			lds		r17, KNOB_SHIFT		
...
MLP_SWITCHSCAN:
			...
			sbrc	r19, SW_KNOB_SHIFT	; Test if 'knob shift' bit is set
			rjmp	SWITCH_UP
			ldi		r16, 1				;
...
SWITCH_UP:								
			...
			ldi		r16, 4
			sts		WRITE_OFFSET, r16	; eeprom offset is 4 (5th byte)
...
; 	
			...
			breq	WRITE_LFODEPTH
			cpi		r16, 6
			breq	WRITE_VCFENVMOD
			cpi		r16, 7
			breq	WRITE_PORTAMENTO

WRITE_LFOFREQ:							; Offset 4
			lds		r17, LFOFREQ
			rjmp	WRITE_BYTE
WRITE_LFODEPTH:							; Offset 5							
			lds		r17, PANEL_LFOLEVEL
			rjmp	WRITE_BYTE
WRITE_VCFENVMOD:						; Offset 6
			lds		r17, VCFENVMOD
			rjmp	WRITE_BYTE
WRITE_PORTAMENTO:						; Offset 7
			lds		r17, PORTAMENTO
			rjmp	WRITE_BYTE
...
WRITE_UPPER:							
			lds		r16, WRITE_OFFSET
			cpi		r16, 1
			breq	WRITE_DCF_ATTACK
			cpi		r16, 2
			breq	WRITE_AMP_DECAY
			cpi		r16, 3
			breq	WRITE_AMP_ATTACK

WRITE_DCF_DECAY:							; Offset 0 
			lds		r17, KNOB_DCF_DECAY
			rjmp	WRITE_BYTE	
WRITE_DCF_ATTACK:							; Offset 1
			lds		r17, KNOB_DCF_ATTACK
			rjmp	WRITE_BYTE	
WRITE_AMP_DECAY:							; Offset 2
			lds		r17, KNOB_AMP_DECAY
			rjmp	WRITE_BYTE	
WRITE_AMP_ATTACK:							; Offset 3
			lds		r17, KNOB_AMP_ATTACK
			rjmp	WRITE_BYTE								
...
WRITE_BYTE:								
			...
			out 	SREG, r19 			; restore SREG value (I-bit)
			cpi		r16, 3				; If eeprom write offset is at the end of knob bank 0 or bank 1, turn off write mode
			breq	CLEAR_WRITE
			cpi		r16, 7
			breq 	CLEAR_WRITE
			inc		r16
...
MLP_VCOX:
		    ...
		    rjmp	MLP_DCF0		    ; exit when LFO=DCO
		    lds	    r16, LFOVALUE		; r16 = LFO	    -128..+127
